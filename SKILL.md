...
Available `fontFamily` ids (Excalidraw 0.18): `3` Cascadia (monospace, this skill's default), `5` Excalifont (hand-drawn, Excalidraw's default when omitted), `1` Virgil (legacy hand-drawn), `6` Nunito, `7` Lilita One, `8` Comic Shanns, `9` Liberation Sans. The renderer rejects every other id: `2` is the deprecated host Helvetica (each OS substitutes its own, making output platform-dependent), and there is no id 4 in Excalidraw itself (the Obsidian Excalidraw plugin assigns id 4 to its user-configured local font, which this renderer cannot render). CJK text is unsupported: the render aborts rather than degrade. Emoji, and any other glyph no vendored font covers (rare symbols, hieroglyphs), are outside that guarantee when mixed with covered text: no font file is requested, the host OS draws them or shows tofu, and output becomes platform-dependent, so avoid them in diagrams.
